"""
Job Cards database module (SQLite + SQLAlchemy) for KMRL.

- Uses ./data/jobcards.db by default (SQLite).
- Set JOBCARDS_DB_URL to point to PostgreSQL/MySQL if needed.
- Provides CSV -> DB import (idempotent), CRUD for admin UI, and a summary helper.

Public API:
  init_db(db_url=None) -> Engine
  upsert_from_csv(path) -> dict
  list_jobcards(limit=200, offset=0) -> dict
  add_jobcard(record: dict) -> dict
  update_jobcard(jc_id: int, patch: dict) -> dict
  delete_jobcard(jc_id: int) -> dict
  summary_for_train(train_id: int) -> dict
"""
...
def _default_db_url() -> str:
    # SQLite file under ./data for easy local deploy
...
def _to_dt(value):
    """Parse common date formats to tz-aware IST datetime (or None)."""
    if value in (None, "", "null", "NaT"):
        return None
    s = str(value).strip()

    # Predictable formats first
    for fmt in (
    ...
    # Fallback to pandas with day-first
...
# ---------------------------------------------------------------------
# CSV → DB import (idempotent; simple natural key to skip duplicates)
# ---------------------------------------------------------------------
def upsert_from_csv(path: str) -> dict:
    """
    Imports jobcards from CSV.

    Natural key to skip duplicates: (trainset_id, title, opened_at)
    """
    init_db()
    p = Path(path)
    if not p.exists():
    ...
    if not (train and stat):
        return {"imported": 0, "skipped": 0, "error": "required columns missing (trainset_id, status)"}

    if stat in raw.columns:
        raw[stat] = raw[stat].astype(str).str.lower()
    if sev and sev in raw.columns:
        raw[sev] = raw[sev].astype(str).str.upper().str[0]
...
# ---------------------------------------------------------------------
# CRUD + listing + summary
# ---------------------------------------------------------------------
def list_jobcards(limit: int = 200, offset: int = 0) -> dict:
    """Return rows formatted for admin table."""
    init_db()
    rows = []
    with SessionLocal() as db:
        q = (
...
def add_jobcard(record: dict) -> dict:
    ...
    if not train:
        return {"error": "trainset_id is required"}
    ...
    jc = JobCard(
        ...
        opened_at=_to_dt(record.get("opened_at")),
        closed_at=_to_dt(record.get("closed_at")),
...
def update_jobcard(jc_id: int, patch: dict) -> dict:
    ...
    with SessionLocal() as db:
        ...
        if "trainset_id" in patch:
            jc.trainset_id = str(patch["trainset_id"]).strip()
        if "status" in patch:
            jc.status = str(patch["status"]).lower()
        if "severity" in patch:
            jc.severity = (str(patch["severity"]).upper().strip()[0] if patch["severity"] else None)
        if "title" in patch:
            jc.title = patch["title"] or None
        if "description" in patch:
            jc.description = patch["description"] or None
        if "opened_at" in patch:
            jc.opened_at = _to_dt(patch["opened_at"])
        if "closed_at" in patch:
            jc.closed_at = _to_dt(patch["closed_at"])
...
def delete_jobcard(jc_id: int) -> dict:
    ...
    with SessionLocal() as db:
        jc = db.get(JobCard, jc_id)
        if not jc:
            return {"deleted": 0, "error": "not found"}
        db.delete(jc)
        db.commit()
        return {"deleted": 1}
...
def summary_for_train(train_id: int) -> dict:
    """
    Aggregated status for a trainset (for dashboard cards):
    - total_jobs, open_jobs, closed_jobs, critical_alert (YES if any open A severity)
    """
    init_db()
    total = open_cnt = closed_cnt = 0
    critical = False
    ...
    return {
        ...
        "critical_alert": "YES" if critical else "NO",
    }